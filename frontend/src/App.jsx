...
const Icons = {
  ...
  User: () => <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>,
  Brain: () => <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 2a5 5 0 00-5 5c0 1.5.75 2.8 1.9 3.6a6 6 0 00-2.8 5.4 6 6 0 0011.8 0 6 6 0 00-2.8-5.4A5 5 0 0017 7a5 5 0 00-5-5z" /></svg>
}

// Agent avatars (simple geometric)
const AgentAvatars = [
  ({className}) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="12 2 22 20 2 20"/></svg>,
  ({className}) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/></svg>,
  ({className}) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 9h6v6H9z"/></svg>,
  ({className}) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="12 2 22 12 12 22 2 12"/></svg>,
]
...
export default function App() {
  ...
  const resetWorkspace = () => {
    setAppView('home'); setActiveSessionId(null); setQuery(''); setReport(''); setFutureReport(''); setFutureOutcomes([]); setLiveTranscript([]); setSources([]); setIsSimulating(false); setProgress(0); setCurrentAgent('Dormant'); setLogs([])
  }
  ...
  const startResearch = async () => {
    ...
    setAppView('processing'); setLogs([]); setProgress(0); setReport(''); setFutureReport(''); setFutureOutcomes([]); setLiveTranscript([])
  ...
  const fetchFinalResults = async (id) => {
    try {
      ...
      const reportRes = await fetch(`${API_BASE}/research/final/${id}`, { headers: { 'Authorization': `Bearer ${token}` } })
      if (reportRes.ok) {
          ...
          <div className="flex-1 overflow-y-auto bg-transparent relative animate-in fade-in duration-1000 scrollbar-hide">
            ...
            <div className={`sticky top-0 z-10 backdrop-blur-2xl border-b px-10 py-6 flex flex-col md:flex-row justify-between md:items-center gap-6 ${isDark ? 'bg-black/60 border-white/5' : 'bg-white/80 border-slate-200 shadow-sm'}`}>
              ...
              <div className="flex gap-4 shrink-0">
                ...
                <button onClick={() => downloadDoc('doc')} className={`flex items-center gap-2 text-[10px] tracking-[0.3em] uppercase px-4 border rounded-full transition-colors ${isDark ? 'text-white/40 hover:text-white border-white/10 hover:bg-white/10' : 'text-slate-500 hover:text-slate-900 border-slate-300 hover:bg-slate-100'}`}>
                  <Icons.Descend /> DOC
                </button>
              </div>
            </div>

            <div className="max-w-4xl mx-auto p-10 lg:p-20 pb-32">
              {/* Report Body */}
              ...
              {/* NEURAL ARENA - Debate Transcript */}
                ...
                <div className="mt-32 relative w-full">
                  ...
                  <div className="flex flex-wrap justify-center gap-8 relative">
                    {uniqueAgents.map((agentName, idx) => {
                      ...
                      return (
                        <div key={agentName} className={`flex-1 min-w-[280px] max-w-[400px] flex flex-col transition-all duration-700 ${isSpeaking ? 'scale-105 opacity-100 z-20' : 'opacity-50 scale-95 z-10'}`}>
